...
DECLARE
    ...
    -- Переменные для хранения данных первого и последнего периодов
...
BEGIN
    ...
    LOOP
        ...
        IF v_total_periods = 1 THEN
            v_first_max_change := v_rec.max_prob_change;
            v_first_coverage   := v_rec.coverage_pct;
            v_first_period_start := v_rec.period_start;
            v_first_period_end   := v_rec.period_end;
        END IF;
        
        -- Сохраняем последний период (перезаписываем на каждой итерации)
        v_last_max_change := v_rec.max_prob_change;
        v_last_coverage   := v_rec.coverage_pct;
        v_last_period_start := v_rec.period_start;
        v_last_period_end   := v_rec.period_end;

        -- Формируем строку таблицы
        v_row := rpad(v_rec.period_start::TEXT || ' – ' || v_rec.period_end::TEXT, 25) ||
                 rpad(round(v_rec.max_prob_change::NUMERIC, 4)::TEXT, 18) ||
                 rpad(v_rec.coverage_pct::TEXT, 14) ||
                 rpad(v_rec.total_trans::TEXT, 20) ||
                 rpad(round(v_rec.stability_factor::NUMERIC, 2)::TEXT, 18);
        v_report := v_report || v_row || E'\n';
    END LOOP;
    ...
    -- Тренд (сравнение первого и последнего периодов)
...
END;
$$;
COMMENT ON FUNCTION report_stability_trend(INT) IS 'Возвращает текстовый отчёт о стабильности вероятностей по 7‑дневным периодам за последние N дней. Включает таблицу с max_prob_change, coverage_pct, total_transitions, stability_factor, описания столбцов и интерпретацию результатов.';
...
DECLARE
    ...
    v_end_ts   TIMESTAMPTZ := (p_end + 1)::TIMESTAMPTZ; -- для функций, ожидающих TIMESTAMPTZ
    v_line_sep CONSTANT TEXT := E'\n---\n';
    v_lookback_days INT;
    v_temp TEXT;
    v_rec RECORD;
...
',
    ...
    -- ------------------------------------------------------------------
    -- 6. Секция: Стабильность вероятностей (report_stability_trend)
    -- ------------------------------------------------------------------
    v_report_lines := array_append(v_report_lines, '');
    ...
    BEGIN
        ...
        LOOP
            v_section_text := v_section_text || format(
                '| %s – %s | %s | %s | %s | %s |' || E'\n',
                v_rec.period_start, v_rec.period_end,
                COALESCE(round(v_rec.max_prob_change::NUMERIC, 4)::TEXT, 'NULL'),
                COALESCE(v_rec.coverage_pct::TEXT, 'NULL'),
                COALESCE(v_rec.total_transitions::TEXT, 'NULL'),
                COALESCE(v_rec.stability_factor::TEXT, 'NULL')
            );
        END LOOP;
        v_report_lines := array_cat(v_report_lines, regexp_split_to_array(v_section_text, E'\n'));
    EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
        -- report_quality_sliding возвращает таблицу, отформатируем в markdown таблицу
