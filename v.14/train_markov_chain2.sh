...
#
# Использование:
#   ./train_markov_chain.sh [--start "YYYY-MM-DD HH:MI:SS"] [--end "YYYY-MM-DD HH:MI:SS"]
#   Если аргументы не указаны, начало определяется как MIN(curr_timestamp) из cluster_stat_median,
#   конец – текущее время UTC.
...
if [[ -n "$START_TIME" ]]; then
    ...
    echo "Начальная дата будет определена как MIN(curr_timestamp) из cluster_stat_median"
fi
...
# =============================================================================
# 0.5. Адаптивная настройка конфигурации (до обучения)
# =============================================================================
...
# Определяем начальную дату для заполнения
if [[ -n "$START_TIME" ]]; then
    FILL_START="$START_TIME"
    echo "Используем заданную начальную дату для performance_history: $FILL_START"
else
    FILL_START=$(psql -t -A -c "SELECT to_char(MIN(curr_timestamp), 'YYYY-MM-DD HH24:MI:SS') FROM cluster_stat_median;" | xargs)
    if [[ -z "$FILL_START" || "$FILL_START" == "0" ]]; then
        echo "Ошибка: нет данных в cluster_stat_median или MIN(curr_timestamp) равен NULL."
        exit 1
    fi
    echo "Начальная дата для performance_history: $FILL_START"
fi

psql -v ON_ERROR_STOP=1 <<EOSQL
DO \$\$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM performance_history WHERE ts >= '$FILL_START'::TIMESTAMPTZ AND ts <= '$END_TIME'::TIMESTAMPTZ LIMIT 1) THEN
...
# Определяем начало и конец для обучения
if [[ -n "$START_TIME" ]]; then
    LEARN_START_EPOCH=$(date -u -d "$START_TIME" +%s 2>/dev/null || echo 0)
else
    LEARN_START_EPOCH=$(psql -t -A -c "SELECT EXTRACT(EPOCH FROM MIN(curr_timestamp))::BIGINT FROM cluster_stat_median;" | xargs)
fi
if [[ -z "$LEARN_START_EPOCH" || "$LEARN_START_EPOCH" -eq 0 ]]; then
    echo "Ошибка: не удалось определить начальную дату для обучения."
    exit 1
fi

LEARN_END_EPOCH=$(date -u -d "$END_TIME" +%s 2>/dev/null || echo 0)
if [[ $LEARN_END_EPOCH -eq 0 ]]; then
...
# =============================================================================
# 6.5. Усиленное забывание для стабилизации вероятностей
# =============================================================================
