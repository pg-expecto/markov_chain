...
# 
# version 14.1
# pg_hazel_train_markov_chain2.sh – гибкое историческое обучение цепи Маркова с возможностью
# указания произвольного периода (--start, --end).
...
# пакетное обучение, подбор горизонта (только при наличии инцидентов),
# забывание, расчёт метрик и проверку.
#
# Использование:
#   ./pg_hazel_train_markov_chain2.sh [--start "YYYY-MM-DD HH:MI:SS"] [--end "YYYY-MM-DD HH:MI:SS"]
#   Если аргументы не указаны, начало определяется как MIN(curr_timestamp)
#   из pgh_stat_cluster_analysis, конец – текущее время UTC.
set -euo pipefail

# ---------------------- Конфигурация подключения к БД -------------------------
#export PGHOST="${PGHOST:-127.0.0.1}"
export PGPORT="${PGPORT:-5432}"
export PGUSER="${PGUSER:-performance_monitoring_user}"
export PGDATABASE="${PGDATABASE:-performance_monitoring_db}"
...
# Если начальная дата не задана, определяем как MIN(curr_timestamp) из pgh_stat_cluster_analysis 
if [[ -z "$START_TIME" ]]; then
    START_TIME=$(psql -t -A -c "SELECT to_char(MIN(curr_timestamp), 'YYYY-MM-DD HH24:MI:SS') FROM pgh_stat_cluster_analysis;" | xargs)
    if [[ -z "$START_TIME" || "$START_TIME" == "0" ]]; then
        echo "Ошибка: нет данных в pgh_stat_cluster_analysis или MIN(curr_timestamp) равен NULL."
        exit 1
    fi
...
# =============================================================================
# 0.5. Установка last_forget_time в начало обучаемого периода (КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ)
# =============================================================================
echo "=== Шаг 0.5: Установка last_forget_time = $START_TIME ==="
psql -v ON_ERROR_STOP=1 <<EOSQL
UPDATE markov_config SET last_forget_time = '$START_TIME'::TIMESTAMPTZ;
EOSQL

# =============================================================================
# 0.6. Гарантировать наличие строки в markov_config (если таблица была пуста)
# =============================================================================
echo "=== Шаг 0.6: Инициализация конфигурации, если таблица пуста ==="
psql -v ON_ERROR_STOP=1 <<EOSQL
INSERT INTO markov_config (
    last_forget_time, alpha, interval_minute,
    transition_log_retention_days, apply_forgetting_log_retention_days,
    profile_comparison_retention_days, adaptive_forgetting_enabled,
    use_adaptive_alpha, base_alpha, min_alpha, incident_half_life_days,
    last_incident_time, min_transitions_for_forgetting,
    forecast_horizon_minutes, min_freq_for_stability,
    js_divergence_threshold
)
SELECT
    '$START_TIME'::TIMESTAMPTZ, 0.1, 360,
    30, 30, 30, TRUE,
    TRUE, 0.05, 0.001, 30.0,
    NULL, 5000,
    30, 300, 0.2
WHERE NOT EXISTS (SELECT 1 FROM markov_config);
EOSQL
...
# =============================================================================
# 7. Динамический подбор горизонта (только если есть инциденты)
# =============================================================================
echo "=== Шаг 7: Проверка наличия инцидентов для подбора горизонта ==="
INCIDENT_COUNT=$(psql -t -A -c "SELECT COUNT(*) FROM performance_incident WHERE start_timepoint BETWEEN '$START_TIME' AND '$END_TIME';")

if [[ "$INCIDENT_COUNT" -eq 0 ]]; then
    echo "⚠️  За период не найдено инцидентов. Подбор горизонта пропускается."
    echo "   Будет использован горизонт из текущей конфигурации."
    # Убедимся, что горизонт установлен (например, 30 минут), если он не задан
    CURRENT_HORIZON=$(psql -t -A -c "SELECT forecast_horizon_minutes FROM markov_config LIMIT 1;")
    if [[ -z "$CURRENT_HORIZON" || "$CURRENT_HORIZON" == "0" ]]; then
        echo "   Устанавливаем горизонт по умолчанию (30 минут)."
        psql -c "UPDATE markov_config SET forecast_horizon_minutes = 30;"
    fi
else
    echo "   Найдено инцидентов: $INCIDENT_COUNT. Запускаем подбор горизонта."
