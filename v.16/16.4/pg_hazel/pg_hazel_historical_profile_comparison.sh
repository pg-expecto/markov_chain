...
DB_NAME="performance_monitoring_db"
DB_USER="performance_monitoring_user"
...
if [[ $ARGC -eq 0 ]]; then
    ...
    START_DATE=$(psql -d "$DB_NAME" -U "$DB_USER" -t -A -c "$SQL_GET_MIN" 2>/dev/null || true)
...
while [[ $(date -d "$current_day" +%Y%m%d 2>/dev/null || echo "0") -le $(date -d "$end_date_only" +%Y%m%d 2>/dev/null || echo "0") ]]; do
    ...
    if psql -d "$DB_NAME" -U "$DB_USER" -c "$SQL_CMD" >> "$LOG_FILE" 2>&1; then
