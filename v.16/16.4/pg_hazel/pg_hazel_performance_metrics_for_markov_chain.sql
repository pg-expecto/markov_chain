...
-- VERSION STATS_PACK
--------------------------------------------------------------------------------
-- pg_hazel_performance_metrics_for_markov_chain.sql
-- version 10.1.5
/*
...
-- с проверкой существования таблицы pgh_stat_cluster_analysis и обработкой ошибок
--------------------------------------------------------------------------------
...
BEGIN
    -- Проверка существования таблицы pgh_stat_cluster_analysis
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'pgh_stat_cluster_analysis'
    ) INTO tbl_exists;
    
    IF NOT tbl_exists THEN
        RAISE DEBUG 'get_current_os_waiting_correlation_for_markov_chain: pgh_stat_cluster_analysis not found, returning defaults';
        RETURN QUERY SELECT 0.0::REAL, 0::SMALLINT, 0::SMALLINT;
        RETURN;
    END IF;

    SELECT MAX(curr_timestamp) INTO timepoint FROM pgh_stat_cluster_analysis;
    ...
    SELECT COALESCE(corr(op_speed_long, waitings_long), 0) INTO speed_waitings_correlation
    ...
    BEGIN
        WITH stats AS (
            ...
            FROM pgh_stat_cluster_analysis s
